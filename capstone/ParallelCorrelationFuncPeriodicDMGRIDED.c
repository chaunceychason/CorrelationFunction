...
int main (int argc, char **argv)
{
   ...
   */
     ...
     //You only want to check scales ~1/10 of the box size.
     //THUS: each box must not be very fine. 10 bins is roughly fine, but should check
     //ESPECIALLY FOR SMALL BOXES. Any grid smaller than 4 is worthless.

    //CHECK IF CHOSEN RESOLUTION IS TOO FINE FOR GRID.
     while ( (1.1*r_max) >= (grid_binmax/grid_numbins)){
        printf("Note: There was an Error with the chosen grid_numbins!:Reselecting numbins...-2\n");
        grid_numbins -= 2;
     }
     //CHECK IF CHOSEN RESOLUTION IS TOO FINE!
     while ( (1.25*r_max) < (grid_binmax/(grid_numbins+2))){
        printf("Note: You can increase grid resoltuion to save time:Raising numbins...+2\n");
        grid_numbins += 2;
     }
     ...
     for(i = 0; i < (N_rand); ++i){
        ...
        gxvar= (int) (floor((randX_LIST[i])*(grid_binwidth)));
     ...
     logactual_D, x1, y1, z1, x2, y2, z2, delx, dely, delz, dist_index, i, j, grid_numbins,grid_x1, grid_y1, grid_z1,\
     grid_x2, grid_y2, grid_z2, grid_x_check,grid_y_check,grid_z_check)
     {
              ...
              if (j!=i){
                 ...
                 */
                 ...
                 if (grid_x_check && grid_y_check && grid_z_check){
                   ...
                   if (fabs(delx) > (3.*grid_binwidth)){
                     printf("ENTERED PBC: X");
                     if (x1 > x2){
                       delx = (x1-(x2+grid_binmax));
                     }
                     else {
                       delx = ((x1+grid_binmax)-x2);
                     }
                   }
                   if (fabs(dely) > (3.*grid_binwidth)){
                     printf("ENTERED PBC: Y");
                     if (y1 > y2){
                       dely = (y1-(y2+grid_binmax));
                     }
                     else {
                  ...
                  if (fabs(delz) > (3.*grid_binwidth)){
                     printf("ENTERED PBC: Z");
                     if (z1 > z2){
                       delz = (z1-(z2+grid_binmax));
                     }
                     else {
                       delx = ((z1+grid_binmax)-z2);
                     }
                   ...
                   if (logD < logr_max){
                     ...
                     if (dist_index >= 0 && dist_index < num_bins_preproc){
                       /*
                       if (dist_index == 1){
                         if ( ((sqrt(pow(10, logD)) < 0.2388) || (sqrt(pow(10, logD)) >  0.3403))){
                           printf("HEY THE VALUE OF THE THING IN BIN 1 %lf \n", sqrt(pow(10,logD)));
                         }
                       }
                       */
   ...
   */
   for(filenum=0; filenum<max_filenum; filenum++){
     ...
     grid_x1, grid_y1, grid_z1, grid_x2, grid_y2, grid_z2, grid_numbins, grid_x_check, grid_y_check, grid_z_check)
     {
        //OMP_NUM_THREADS = 16; 
              ...
              if ( j!=i ){
                ...
                /*if (grid_x1==15 && grid_x2==0){
                 ...
                 if (grid_x_check && grid_y_check && grid_z_check){
                   ...
                   dely = y1 - y2;
                   delz = z1 - z2;
                   if ( ( (fabs(delx) > (2.*grid_binwidth)) || (fabs(dely) > (2.*grid_binwidth)) || (fabs(delz) > (2.*grid_binwidth)))){
                      printf("del_x, y, z: %lf %lf %lf \n", fabs(delx), fabs(dely), fabs(delz));
                   }
                   //CORRECT FOR PERIOD BOUNDARY CONDITIONS

                   if ((grid_x1==0 && grid_x2==(grid_numbins-1)) || (grid_x2==0 && grid_x1==(grid_numbins-1))){
                      printf("X Grid value is spanning the box. in the gridchecksection. this isnt necessarily bad.");
                   }

                   if ((grid_y1==0 && grid_y2==(grid_numbins-1)) || (grid_y2==0 && grid_y1==(grid_numbins-1))){
                      printf("Y Grid value is spanning the box. in the gridchecksection. this isnt necessarily bad.");
                   }
                   if ((grid_z1==0 && grid_z2==(grid_numbins-1)) || (grid_z2==0 && grid_z1==(grid_numbins-1))){
                      printf("Z Grid value is spanning the box. in the gridchecksection. this isnt necessarily bad.");
                   }


                   if (fabs(delx) > (2.*grid_binwidth)){
     ...
     */
     ...
     for ( i=0 ; i <= num_bins_preproc ; i++ ) {
        //fprintf(fp_out,"%ld \n", distance_counts[i]);
        //dist_index = (int) (floor((logD - logr_min)*((num_bins_preproc)/(logr_max - logr_min))));
        //bin_counts[i] = sqrt(pow(10, (i * ((logr_max - logr_min) / (num_bins_preproc+1)) + logr_min)));
        bin_counts[i] = sqrt(pow(10, ((logr_max - logr_min)/num_bins_preproc ) * i + logr_min));
        //bin_counts[i] = (i+1) * ((logr_max - logr_min)/num_bins_preproc);
        fprintf(fp_out, "%lf \n", bin_counts[i]);
     }
     fclose(fp_out);
     ...
     */
     ...
     //FILE *fp_out;
     fp_out = fopen("output_DMcountsVariableResearchGRID.txt","w");
     if ( fp_out == NULL ) {
        printf("output_file.txt not opened, exiting...\n");
        exit(0);
     }
