...
int main (int argc, char **argv)
{
   ...
   */
    ...
    //CHECK IF CHOSEN RESOLUTION IS TOO FINE FOR GRID.
     /*while ( (1.2*r_max) >= (grid_binmax/grid_numbins)){
        printf("Note: There was an Error with the chosen grid_numbins!:Reselecting numbins...-1\n");
        grid_numbins -= 1;
     }
     //CHECK IF CHOSEN RESOLUTION IS TOO FINE!
     while ( (1.25*r_max) < (grid_binmax/(grid_numbins+1))){
        printf("Note: You can increase grid resoltuion to save time:Raising numbins...+1\n");
        grid_numbins += 1;
     }
